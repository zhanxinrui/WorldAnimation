...
function createArcsRocket(callbackFn) {
    ...
    for ( i = 0; i < dataMap.length-1; i ++ ) {
        var p1 = latLongToVector3(dataMap[0][2], dataMap[0][3], globeRadius, 0.5);
        var p4 = latLongToVector3(dataMap[i+1][2], dataMap[i+1][3], globeRadius, 0.5);
...
createArcsRocket();
renderArcsRocket();
await scene.add(arcRocketObject);
